  ...
  function chooseEpisode(courseId, preview = false) {
    ...
    openModal(mode === 'adult' ? 'AFTER CLASS / 成年向け' : 'CAMPUS STORY / 未成年向け', '今日の物語を選ぼう',
      `<p class="course-intro">${'どの物語からでも、初めて遊べます。'}<br><span>1編約5分・ミニ問題3問・結末3種類。${'物語ごとに、学べる内容も変わります。'}</span></p><div class="episode-list">${S.episodes[mode].map((entry, i) => {
        ...
        return `<button class="episode-option${mode === 'adult' ? ' adult-episode' : ''}" data-episode="${entry.id}"><span class="episode-top"><span class="episode-id">${icon(({lecture: 'book', festival: 'spark', rescue: 'archive', rain: 'rain', presentation: 'presentation'})[entry.id])}<span>STORY 0${i + 1}</span></span><span class="episode-status">${i ? 'NEW' : 'ORIGINAL'}</span></span><strong>${escape(meta.title)}</strong><span class="episode-description">${escape(meta.description)}</span><span class="episode-topics">${S.course(selected).lessons.map(l => escape(l.title)).join(' / ')}</span><span class="episode-link"><span>${preview ? 'この物語の学びを見る' : 'この物語をはじめる'}</span><span class="card-arrow">${icon('arrow')}</span></span></button>`;
